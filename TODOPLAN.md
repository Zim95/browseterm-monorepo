...
- [ ] **`status_monitor`'s pod-watcher can miss a pod deletion entirely** if the pod disappears while its own connection to the k8s API is itself down (e.g. a local network blip on a user's machine): the DELETED watch event is never delivered, and the post-recovery reconcile only lists pods that currently exist — it never diffs against previously-known container_ids to notice one silently dropped out, so `mark_lost_if_running` is never called and the container's row stays stuck `RUNNING` forever with no live pod behind it (reproduced 2026-09-18; fixed the one stuck row by hand via Cloud's `hibernate_container` endpoint). Fix: have the reconcile step diff the fresh pod list against the previous `seen` set and route anything that dropped out through the same guarded-hibernate path DELETED events use.
- [ ] **No database backups.** Need Postgres WAL archiving (`archive_mode=on` + `archive_command`) plus periodic `pg_basebackup` base backups, shipped somewhere that survives losing the Contabo VM entirely — not just another path on its own disk. Blocked on the infra-repo consolidation below (backup tooling config belongs there); pick the actual storage target (S3-compatible object storage recommended over GitHub, which isn't built for frequent binary WAL churn) once that repo exists.
- [ ] **New `infra` repo**: consolidate `postgres_ha` and `redis_ha` out of `browseterm-monorepo` *and* out of `~/ikompare` into one dedicated repo holding the k3s setup + DB/Redis config, installable on a fresh VM. Later: extend it to support our own VM cluster setup too. Explicitly deferred — do this after the current project work is finished, not now.
